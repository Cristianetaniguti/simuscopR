...
docker run -v $(pwd):/opt/ cristaniguti/vcftools vcftools --gzvcf /opt/vcf.file.gz --indv ID1 --chr Chr01 --recode --out /opt/vcf_filt
```

To obtain the BED file from the BAM file

```{bash}
docker run -it -v $(pwd):/data  biocontainers/bedtools:v2.28.0_cv1 bamtobed -i /data/filtered_bam > bed_file
```

# Create profile

```{r}
seqToProfile("PT_F.chr10.sorted.bam", "bed_file", "vcf_chr10.recode.vcf",
             "../reference/Chr10.populus.fa", "PT_F.profile")

AdaptBed(bed.file = "PT_F.sorted_filt.bed", out.file = "PT_F.sorted_filt_edit3.bed", int.size = 500)
```
...
/data/reference/Ptrichocarpa_533_v4.0.fa /data/two_enzy/P1_100_150_1.fq.gz | java -jar /picard.jar SortSam \
I="file.bam" \
O="/data/P1_2enz2.bam" \
TMP_DIR=./tmp \
